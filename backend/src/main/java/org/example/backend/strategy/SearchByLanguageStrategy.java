...
@Component
public class SearchByLanguageStrategy implements MovieSearchStrategy {

    @Override
    public boolean isApplicable(MovieSearchRequest request) {
        return request.getLanguage() != null && !request.getLanguage().trim().isEmpty();
    }

    @Override
    public Specification<Movie> getSpecification(MovieSearchRequest request) {
        String pattern = "%" + request.getLanguage().trim().toLowerCase() + "%";
        return (root, query, cb) -> cb.like(cb.lower(root.get("language")), pattern);
    }
}
